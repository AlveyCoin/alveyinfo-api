...
class AddressService extends Service {
  async getAddressSummary(addressIds, p2pkhAddressIds, rawAddresses) {
    const {Address} = this.ctx.app.alveyinfo.lib
  ...
  async getAddressTransactionCount(addressIds, rawAddresses) {
    const {Address: RawAddress, Solidity} = this.app.alveyinfo.lib
    ...
    let [{count}] = await db.query(sql`
      SELECT COUNT(*) AS count FROM (
        ...
        WHERE receipt._id = log.receipt_id
          ...
          AND contract.address = log.address AND contract.type IN ('arc20', 'arc721')
          AND log.topic1 = ${TransferABI.id}
          AND (log.topic2 IN ${topics} OR log.topic3 IN ${topics})
          AND (
            (contract.type = 'arc20' AND log.topic3 IS NOT NULL AND log.topic4 IS NULL)
            OR (contract.type = 'arc721' AND log.topic4 IS NOT NULL)
          )
      ) list
  ...
  async getAddressARC20TokenTransactionCount(rawAddresses, token) {
    const {Address, Solidity} = this.app.alveyinfo.lib
    const TransferABI = Solidity.arc20ABIs.find(abi => abi.name === 'Transfer')
  ...
  async getAddressARC20TokenTransactions(rawAddresses, token) {
    const {Address, Solidity} = this.app.alveyinfo.lib
    const TransferABI = Solidity.arc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    ...
    let topicAddresses = rawAddresses
      .filter(address => address.type === Address.PAY_TO_PUBLIC_KEY_HASH)
      .map(address => Buffer.concat([Buffer.alloc(12), address.data]))
    let totalCount = await this.getAddressARC20TokenTransactionCount(rawAddresses, token)
    let transactions = await db.query(sql`
      SELECT
      ...
      INNER JOIN arc20 ON arc20.contract_address = log.address
  ...
  async getAddressARC20TokenMempoolTransactions(rawAddresses, token) {
        ...
        },
        {
          ...
          include: [{
            ...
            include: [{
              ...
              as: 'contract',
              required: true,
              where: {address: token.contractAddress, type: 'arc20'},
              attributes: []
            }]
          }]
        },
