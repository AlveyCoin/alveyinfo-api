...
class TransactionService extends Service {
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.alveyinfo.lib
  ...
  async transformTransaction(transaction, {brief = false} = {}) {
    ...
    let [arc20TokenTransfers, arc20TokenUnconfirmedTransfers, arc721TokenTransfers] = await Promise.all([
      this.transformARC20Transfers(transaction.outputs),
      confirmations === 0 ? this.transformARC20UnconfirmedTransfers(transaction.outputs) : undefined,
    ...
    return {
      ...
      arc20TokenTransfers,
      arc20TokenUnconfirmedTransfers,
      arc721TokenTransfers
    }
  }

  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.alveyinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.alveyinfo.lib
  ...
  async transformARC20Transfers(outputs) {
    ...
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, data, arc20} of output.evmReceipt.logs) {
  ...
  async transformARC721Transfers(outputs) {
    ...
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, arc721} of output.evmReceipt.logs) {
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.alveyinfo.lib
    const {
