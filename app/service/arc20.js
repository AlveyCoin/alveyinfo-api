...
class ARC20Service extends Service {
  async listARC20Tokens() {
    ...
    const {Arc20Statistics: ARC20Statistics} = db
    const {sql} = this.ctx.helper
    const {gt: $gt} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await ARC20Statistics.count({
    ...
    let list = await db.query(sql`
      ...
      FROM (
        SELECT contract_address, holders, transactions FROM arc20_statistics
  ...
  async getAllARC20Balances(hexAddresses) {
    ...
    const {
      ...
      Contract, EvmReceipt: EVMReceipt, Arc20: ARC20, Arc20Balance: ARC20Balance,
      where, col
    } = this.ctx.model
      ...
      if (byteCode.length === 68
        ...
        if (mapping.has(item.output.address.contract.addressString)) {
          ...
          data = {
            ...
            name: item.output.address.contract.arc20.name,
            symbol: item.output.address.contract.arc20.symbol,
            decimals: item.output.address.contract.arc20.decimals,
            balance: 0n,
            unconfirmed: {
              received: 0n,
              sent: 0n
  ...
  async getARC20Balance(rawAddresses, tokenAddress) {
    const {Address: RawAddress, OutputScript, Solidity} = this.app.alveyinfo.lib
    const transferABI = Solidity.arc20ABIs.find(abi => abi.name === 'transfer')
    const {
      Address, TransactionOutput,
      Contract, EvmReceipt: EVMReceipt, Arc20: ARC20, Arc20Balance: ARC20Balance,
  ...
  async getARC20BalanceHistory(addresses, tokenAddress) {
    ...
    const {
      ...
      Contract, Arc20: ARC20, Arc20Balance: ARC20Balance,
      literal
    } = db
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM evm_receipt receipt, evm_receipt_log log, arc20
      WHERE receipt._id = log.receipt_id AND log.address = arc20.contract_address AND ${{raw: logFilter}}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    if (totalCount === 0) {
      return {totalCount: 0, transactions: []}
    }
    let ids = (await db.query(sql`
      ...
      INNER JOIN (
        SELECT DISTINCT(receipt.transaction_id) AS id FROM evm_receipt receipt, evm_receipt_log log, arc20
        WHERE receipt._id = log.receipt_id AND log.address = arc20.contract_address AND ${{raw: logFilter}}
  ...
  async getAllARC20TokenTransactions() {
    const TransferABI = this.app.alveyinfo.lib.Solidity.arc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    let {limit, offset, reversed = true} = this.ctx.state.pagination
    ...
    let [{totalCount}] = await db.query(sql`
      ...
      FROM arc20, evm_receipt_log log
      WHERE arc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    let transactions = await db.query(sql`
      SELECT
        ...
        arc20.name AS name,
        arc20.symbol AS symbol,
        arc20.decimals AS decimals,
        evm_receipt_log.topic2 AS topic2,
        evm_receipt_log.topic3 AS topic3,
        evm_receipt_log.data AS data
      FROM (
        SELECT log._id AS _id FROM arc20, evm_receipt_log log
        WHERE arc20.contract_address = log.address AND log.topic1 = ${TransferABI.id}
      ...
      INNER JOIN arc20 ON arc20.contract_address = evm_receipt_log.address
  ...
  async getARC20TokenTransactions(contractAddress) {
    const TransferABI = this.app.alveyinfo.lib.Solidity.arc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
  ...
  async getARC20TokenRichList(contractAddress) {
    const db = this.ctx.model
    const {Arc20Balance: ARC20Balance} = db
  ...
  async updateARC20Statistics() {
    const TransferABI = this.app.alveyinfo.lib.Solidity.arc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {Arc20: ARC20, Arc20Statistics: ARC20Statistics} = db
    const {sql} = this.ctx.helper
    let transaction = await db.transaction()
    try {
      let result = (await ARC20.findAll({attributes: ['contractAddress'], transaction})).map(
        ({contractAddress}) => ({contractAddress, holders: 0, transactions: 0})
      )
      let balanceResults = await db.query(sql`
        SELECT contract_address AS contractAddress, COUNT(*) AS count FROM arc20_balance
      ...
      await db.query(sql`DELETE FROM arc20_statistics`, {transaction})
      await ARC20Statistics.bulkCreate(result, {validate: false, transaction, logging: false})
