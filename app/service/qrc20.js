...
class ARC20Service extends Service {
  async listARC20Tokens() {
    ...
    const {Qrc20Statistics: ARC20Statistics} = db
  ...
  async getAllARC20Balances(hexAddresses) {
    ...
    const {
      ...
      Contract, EvmReceipt: EVMReceipt, Qrc20: ARC20, Qrc20Balance: ARC20Balance,
      where, col
    } = this.ctx.model
  ...
  async getARC20BalanceHistory(addresses, tokenAddress) {
    ...
    const {
      ...
      Contract, Qrc20: ARC20, Qrc20Balance: ARC20Balance,
      literal
    } = db
  ...
  async updateARC20Statistics() {
    const TransferABI = this.app.alveyinfo.lib.Solidity.arc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {Qrc20: ARC20, Qrc20Statistics: ARC20Statistics} = db
    const {sql} = this.ctx.helper
