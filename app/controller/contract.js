...
class ContractController extends Controller {
  async summary() {
    ...
    ctx.body = {
      ...
      ...summary.type === 'arc20' ? {
        arc20: {
          name: summary.arc20.name,
          symbol: summary.arc20.symbol,
          decimals: summary.arc20.decimals,
          totalSupply: summary.arc20.totalSupply.toString(),
          version: summary.arc20.version,
          holders: summary.arc20.holders,
          transactions: summary.arc20.transactions
        }
      } : {},
      ...summary.type === 'arc721' ? {
        arc721: {
          name: summary.arc721.name,
          symbol: summary.arc721.symbol,
          totalSupply: summary.arc721.totalSupply.toString()
        }
      } : {},
  ...
  async arc20BalanceHistory() {
    ...
    let {totalCount, transactions} = await ctx.service.arc20.getARC20BalanceHistory([ctx.state.contract.contractAddress], tokenAddress)
    ctx.body = {
      totalCount,
      transactions: transactions.map(tx => ({
        id: tx.id.toString('hex'),
