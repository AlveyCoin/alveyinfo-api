...
class AddressController extends Controller {
  async summary() {
    ...
    ctx.body = {
      ...
      arc20Balances: summary.arc20Balances.map(item => ({
      ...
      arc721Balances: summary.arc721Balances.map(item => ({
        address: item.addressHex.toString('hex'),
        addressHex: item.addressHex.toString('hex'),
        name: item.name,
        symbol: item.symbol,
  ...
  async arc20TokenBalance() {
    let {ctx} = this
    let {address, token} = ctx.state
    if (token.type !== 'arc20') {
      ctx.body = {}
    }
    let {name, symbol, decimals, balance, unconfirmed} = await ctx.service.arc20.getARC20Balance(address.rawAddresses, token.contractAddress)
    ctx.body = {
      name,
      symbol,
      decimals,
  ...
  async arc20TokenTransactions() {
    let {ctx} = this
    let {address, token} = ctx.state
    let {totalCount, transactions} = await ctx.service.address.getAddressARC20TokenTransactions(address.rawAddresses, token)
    ctx.body = {
      totalCount,
      transactions: transactions.map(transaction => ({
        transactionId: transaction.transactionId.toString('hex'),
  ...
  async arc20TokenMempoolTransactions() {
    let {ctx} = this
    let {address, token} = ctx.state
    let transactions = await ctx.service.address.getAddressARC20TokenMempoolTransactions(address.rawAddresses, token)
  ...
  async arc20BalanceHistory() {
    const {Address} = this.app.alveyinfo.lib
    let {ctx} = this
    let tokenAddress = null
    if (ctx.state.token) {
      if (ctx.state.token.type === 'arc20') {
    ...
    let {totalCount, transactions} = await ctx.service.arc20.getARC20BalanceHistory(hexAddresses, tokenAddress)
    ctx.body = {
      totalCount,
      transactions: transactions.map(tx => ({
        id: tx.id.toString('hex'),
